<template>
  <view>
    ...
    <!-- 商品列表 -->
    <view v-if="showNoData" >暂未找到该商品!</view>
    <scroll-view v-else class="goods" scroll-y>
      <view 
        class="item"
        v-for="good in list"
        :key="good.goods_id"
...
export default {
  ...
  computed: {
    showNoData () {
      ...
      return !this.list.length && pagenum === 1 && !loading
    }
  },
  methods: {
    goDetail () {
  ...
  methods: {
    async getList () {
      ...
      try {
        let { pagenum, pagesize, total, list, search, loading } = this
        this.loading = true
