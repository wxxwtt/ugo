  ...
  <view>
    ...
    <!-- 商品列表 -->
      ...
      <scroll-view v-else class="goods" scroll-y @scrolltolower="reachBottomHandle">
        <view class="item" v-for="good in list" :key="good.goods_id" @click="goDetail(good.goods_id)">
          <!-- 商品图片 -->
          <image v-if="!good.goods_small_logo" class="pic" :src="good.goods_small_logo"></image>
          <image v-else class="pic" :src="good.goods_small_logo" binderror="errorImage"></image>
          <!-- 商品信息 -->
          <view class="meta">
...
export default {
  ...
  methods: {
    ...
    errorImage (index) {
      this.list[index].goods_small_logo = defaultImageURL
    }
  },
  onLoad (params) {
    const { name } = params
