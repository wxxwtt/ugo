    ...
    <view class="category">
      ...
      <view class="sub">
        <scroll-view scroll-y>
          ...
          <view class="children" 
            v-for="category in activeCategory"
            :key="category.cat_id">
            <view class="title">{{ category.cat_name }}</view>
            <!-- 品牌 -->
            <view class="brands">
              <navigator 
                :url="'/pages/list/index?name=' + brand.cat_name"
                v-for="brand in category.children"
                :key="brand.cat_id"
                >
...
export default {
  ...
  computed: {
    activeCategory () {
      return this.category.length && this.category[this.topCategoryIndex].children
    }
  },
  methods: {
    async getCategory () {
